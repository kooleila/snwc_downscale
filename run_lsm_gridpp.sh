...
#lat2=$3 # second latitude for plotting
#lon1=$4 # first longitude for plotting
#lon2=$5 # second longitude for plotting
shift
WEEKDAY=`date +"%a"`
HOD=`date +"%H"`
...
python3 lsm_gridpp.py --topography_data "$bucket""$NN"00/Z-M2S2.grib2 --landseacover "$bucket""$NN"00/LC-0TO1.grib2 --parameter_data $datafile --output testi_"$parameter".grib2 --parameter "$pyparam" "$@"
